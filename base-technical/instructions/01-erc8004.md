...
- `name` -- human-readable name (e.g., "BAiSED")
- `description` -- what the agent does
- `services` -- map of service types -> endpoints (REST, AI2AI, x402)
- `x402Support` -- boolean, whether agent accepts x402 payments
- `active` -- boolean, whether agent is currently operational
- `registrations` -- timestamped registration entries

**Optional fields:**
- `capabilities` -- list of agent capabilities
- `preferredPaymentCurrency` -- e.g., "USDC"
- `agentKey` -- public key for verification
- `discovery` -- discovery metadata
- `nameService` -- ENS/Basename mapping
...
2. **Reputation:** Structured feedback -- ratings, comments, timestamps. Agents request feedback after tasks.
...
- Enables reverse lookups: Basename -> agent identity -> capabilities
