...
## Payment Flow
1. **Client request:** `GET /data` â normal HTTP request
...
- `currency` â token address
- `acceptedMethods` â e.g., `["erc3009", "permit2"]`
- `expiry` â payment deadline
- `paymentId` â opaque identifier
- `paymentPurpose` â human-readable description (optional)
...
Settlement proof â transaction hash or facilitator receipt.

## Payment Mechanisms
- **EIP-3009** (`transferWithAuthorization`) â gasless token transfers, primary method
- **Permit2** â alternative authorization scheme
...
- If absent â responds with 402 + `PAYMENT-REQUIRED`
- If present â verifies via facilitator contract, settles payment
